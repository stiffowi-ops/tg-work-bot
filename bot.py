...
class FactScheduler:
    ...
    def __init__(self, current_index: int = 0):
        ...
        logger.info(f"Инициализирован планировщик фактов с индексом {current_index}")
    ...
    def get_wikipedia_fact(self, category: str, lang: str = 'ru') -> Tuple[str, str, str]:
        ...
        try:
            ...
            category_keywords = {
                'музыка': ['музыка', 'песня', 'альбом', 'исполнитель', 'группа', 'композитор', 'музыкант'],
                'фильмы': ['фильм', 'кинематограф', 'режиссёр', 'актёр', 'кино', 'сериал', 'голливуд'],
                'технологии': ['технология', 'компьютер', 'программа', 'интернет', 'наука', 'изобретение', 'инновация'],
                'игры': ['игра', 'видеоигра', 'компьютерная игра', 'разработчик', 'игрок', 'консоль', 'игровая индустрия']
            }
            
            # Используем несколько ключевых слов для лучшего поиска
            ...
            params = {
                ...
                'format': 'json',
                'list': 'search',
                'srsearch': f'intitle:"{search_keyword}"',
                'srlimit': 50,
                'srwhat': 'nearmatch',
                'srinfo': 'totalhits',
            }
            ...
            if 'query' not in data or not data['query']['search']:
                ...
                # Пробуем без intitle
                params['srsearch'] = search_keyword
                response = requests.get(url, params=params, headers=headers, timeout=15)
                response.raise_for_status()
                data = response.json()
            ...
            # Шаг 2: Получаем содержание статьи (полный текст)
            params = {
                'action': 'query',
                'format': 'json',
                'prop': 'extracts|info',
                'inprop': 'url',
                'exchars': 1500,  # Достаточно для полного факта
                'explaintext': True,
                'exintro': True,  # Только вводную часть
                'titles': title
            }
            ...
            # ОБРАБОТКА: Убеждаемся, что факт заканчивается на законченном предложении
            if fact:
                # Удаляем технические пометки Wikipedia
                fact = fact.replace('\n', ' ').strip()
                ...
                # Если есть законченные предложения, берем их все
                if sentences:
                    fact = ' '.join(sentences)
                # Иначе добавляем точку в конце, если ее нет
                elif fact and fact[-1] not in sentence_endings:
                    fact = fact + '.'
                
                # Обрезаем до разумной длины, но только на границе предложения
                if len(fact) > 1200:
                    # Ищем последнее законченное предложение до 1200 символов
                    truncated = fact[:1200]
                    last_period = max(
                        truncated.rfind('.'),
                        truncated.rfind('!'),
                    ...
                    if last_period > 800:  # Минимум 800 символов для осмысленного факта
                        fact = truncated[:last_period + 1]
                    else:
                        # Если не нашли хорошую точку обрыва, используем fallback
                        return self._get_fallback_fact(category), "", title
            ...
            logger.info(f"Успешно получен факт: {title} ({len(fact)} символов)")
    ...
    def _get_fallback_fact(self, category: str) -> str:
        ...
        fallback_facts = {
            'музыка': [
                "Бетховен продолжал сочинять музыку даже после того, как полностью потерял слух. Его знаменитая Девятая симфония была создана, когда он был уже полностью глухим.",
                "Группа The Beatles удерживает рекорд по количеству проданных альбомов в истории музыки. Их альбом '1' разошёлся тиражом более 31 миллиона копий по всему миру.",
                "Скрипка Страдивари может стоить более 15 миллионов долларов. Антонио Страдивари создал около 1100 инструментов, из которых сохранилось около 650.",
                "Песня 'Happy Birthday to You' изначально называлась 'Good Morning to All'. Авторами были сёстры Милдред и Патти Хилл в 1893 году.",
                "В опере 'Волшебная флейта' Моцарта используется музыкальная тема, основанная на масонских символах. Моцарт был активным членом масонской ложи."
            ],
            'фильмы': [
                "Первый в истории полнометражный фильм был снят в 1906 году и длился около 60 минут. Это был австралийский фильм 'История банды Келли'.",
                "Альфред Хичкок никогда не получал премию 'Оскар' за лучшую режиссуру. Он получил только почетную награду в 1968 году за вклад в киноискусство.",
                "Фильм 'Аватар' Джеймса Кэмерона является самым кассовым фильмом в истории кино. Его сборы составили более 2,8 миллиарда долларов.",
                "Для съёмок 'Властелина колец' было изготовлено более 48 000 предметов реквизита. Включая 10 000 стрел, 1800 пар обуви хоббитов и 100 специально выращенных деревьев.",
                "Мэрилин Монро имела IQ 168, что считается уровнем гения. Она также была соучредителем собственной кинокомпании в 1955 году."
            ],
            'технологии': [
                "Первый компьютерный вирус был создан в 1983 году и назывался 'Elk Cloner'. Он распространялся через дискеты для компьютеров Apple II.",
                "QR-коды были изобретены в Японии в 1994 году для отслеживания запчастей автомобилей. Создатель Масахиро Хара разработал их для компании Denso Wave.",
                "Средний смартфон сегодня имеет больше вычислительной мощности, чем компьютеры NASA в 1969 году. Апполон-11 управлялся компьютером с 64 КБ памяти.",
                "Первое в мире SMS было отправлено в 1992 году и содержало текст 'Счастливого Рождества!'. Его отправил инженер Нейл Папуорт с компьютера на мобильный телефон.",
                "Искусственный интеллект впервые обыграл чемпиона мира по шахматам в 1997 году. Компьютер Deep Blue победил Гарри Каспарова со счётом 3,5:2,5."
            ],
            'игры': [
                "Первая в мире видеоигра была создана в 1958 году и называлась 'Tennis for Two'. Её разработал физик Уильям Хигинботам на осциллографе.",
                "Персонаж Марио изначально назывался 'Прыгающий человек' и появился в игре 'Donkey Kong'. Своё имя он получил в честь владельца склада Nintendo Америки.",
                "Игра Minecraft является самой продаваемой видеоигрой в истории. Было продано более 300 миллионов копий по всему миру.",
                "Разработчики игры The Legend of Zelda вдохновлялись детскими воспоминаниями о лесах Киото. Сигеру Миямото создал игру на основе своих приключений в детстве.",
                "Самый дорогой предмет в истории игр - космический корабль в Eve Online, проданный за 330 тысяч долларов. Это был корабль класса 'Титан' альянса Band of Brothers."
            ]
        }
    ...
    def create_fact_message(self, category: str) -> Tuple[str, InlineKeyboardMarkup]:
        ...
        keyboard = []
        row = []
        for i, emoji in enumerate(FACT_REACTIONS):
            # Используем более простой и понятный callback_data
            # Формат: fact_<category_code>_<emoji_index>
            category_code = {
                'музыка': 'mu',
                'фильмы': 'fi',
                'технологии': 'te',
            ...
            callback_data = f"fact_{category_code}_{i}"
            row.append(
...
class BotConfig:
    ...
    def get_fact_scheduler(self) -> FactScheduler:
        """Получаем планировщик фактов с сохраненным индексом"""
        return FactScheduler(current_index=self.fact_current_index)
...
async def send_daily_fact(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Получаем планировщик с сохраненным индексом
        fact_scheduler = config.get_fact_scheduler()
        
        # Получаем текущую категорию
        category = fact_scheduler.get_next_category()
        logger.info(f"Отправка факта категории: {category}, индекс: {fact_scheduler.current_index}")
        ...
        new_index = config.increment_fact_index()
        logger.info(f"✅ Факт отправлен: {category}. Следующий индекс: {new_index}")
...
async def handle_fact_reaction(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        # Парсим callback_data: fact_mu_0 (где mu - код категории, 0 - индекс реакции)
        parts = query.data.split('_')
        if len(parts) == 3 and parts[0] == "fact":
            # Определяем категорию по коду
            category_code = parts[1]
            category_map = {'mu': 'музыка', 'fi': 'фильмы', 'te': 'технологии', 'ga': 'игры'}
            category = category_map.get(category_code, 'музыка')
            
            # Получаем индекс реакции
            try:
                reaction_idx = int(parts[2])
                if 0 <= reaction_idx < len(FACT_REACTIONS):
                    emoji = FACT_REACTIONS[reaction_idx]
                else:
                    emoji = '👍'
            except ValueError:
                emoji = '👍'
            ...
            logger.debug(f"Реакция на факт: {emoji} на категорию {category} от @{query.from_user.username}")
            
    except Exception as e:
        logger.error(f"Ошибка обработки реакции на факт: {e}, data: {query.data}")
...
async def send_fact_now(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Получаем текущую категорию
        category = fact_scheduler.get_next_category()
        logger.info(f"Отправка факта по команде: {category}, индекс: {fact_scheduler.current_index}")
        ...
        # Увеличиваем индекс для следующего факта
        new_index = config.increment_fact_index()
        logger.info(f"Факт отправлен по команде: {category}. Следующий индекс: {new_index}")
...
async def show_next_fact_category(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    next_index = (fact_scheduler.current_index + 1) % len(FACT_CATEGORIES)
    next_category = FACT_CATEGORIES[next_index]
    ...
    response = f"📚 *Информация о фактах:*\n\n"
    ...
    try:
        ...
        if delay > 0:
            ...
            if not existing_jobs:
                ...
                logger.info(f"Следующая категория: {FACT_CATEGORIES[config.fact_current_index]}")
            else:
                logger.info(f"Отправка факта на {next_time} уже запланирована")
        else:
            # Если время уже прошло, планируем на следующий день
            logger.warning(f"Время отправки факта уже прошло ({next_time}), планируем на следующий день")
            context.application.job_queue.run_once(
                schedule_next_fact,
                60,  # Через минуту
                chat_id=chat_id
        ...
        logger.error(f"Ошибка планирования факта: {e}")
        # Пробуем снова через 5 минут
        context.application.job_queue.run_once(
            schedule_next_fact,
            300,
...
def main() -> None:
    ...
    try:
        ...
        # Обработчик реакций на факты - ВАЖНО: правильный pattern
        application.add_handler(
            CallbackQueryHandler(handle_fact_reaction, pattern="^fact_.+")
        )
        ...
        logger.info(f"👍 Реакции на факты: {', '.join(FACT_REACTIONS)}")
