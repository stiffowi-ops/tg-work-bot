...
class FactScheduler:
    ...
    def get_wikipedia_fact(self, category: str, lang: str = 'ru') -> Tuple[str, str, str]:
        ...
        try:
            ...
            # Правильно формируем URL (используем quote для кодирования)
    ...
    def create_fact_message(self, category: str) -> Tuple[str, InlineKeyboardMarkup]:
        ...
        # Создаем inline-кнопки с реакциями (компактный формат)
        keyboard = []
        row = []
        for i, emoji in enumerate(FACT_REACTIONS):
            # Используем более компактный callback_data
            callback_data = f"fact_{category[:2]}_{i}"  # Например: fact_mu_0
            row.append(
...
class BotConfig:
    ...
    def __init__(self):
        ...
        self.fact_scheduler = FactScheduler()  # Единый экземпляр для всех
...
async def send_daily_fact(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Используем единый планировщик из конфига
        fact_scheduler = config.fact_scheduler
...
async def handle_fact_reaction(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        # Парсим callback_data: fact_mu_0 (где mu - первые 2 буквы категории, 0 - индекс реакции)
        parts = query.data.split('_')
        if len(parts) >= 3:
            # Определяем категорию по короткому коду
            category_code = parts[1]
            category_map = {'mu': 'музыка', 'fi': 'фильмы', 'te': 'технологии', 'ig': 'игры'}
...
async def show_next_fact_category(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    response += f"📅 Будет отправлена {moscow_time.strftime('%d.%m.%Y в %H:%M')} по МСК"
...
def main() -> None:
    ...
    try:
        ...
        application.add_handler(
            CallbackQueryHandler(handle_fact_reaction, pattern="^fact_.+_.+$")
        )
