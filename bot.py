...
PROFILE_INTERESTS = [
    ...
    ("outdoors", "🥾 Походы и природа"),
    ("cooking", "🍳 Кулинария"),
    ("food", "☕ Кофе и гастрономия"),
    ...
    ("events", "🎭 Театр и концерты"),
    ("wellness", "🧘 ЗОЖ и медитация"),
    ("volunteering", "🤝 Волонтёрство"),
]
PROFILE_INTEREST_LABELS = dict(PROFILE_INTERESTS)
PROFILE_INTEREST_KEYS = set(PROFILE_INTEREST_LABELS)

def normalize_profile_interests(values) -> list[str]:
    """Оставляет только разрешённые уникальные интересы, сохраняя порядок каталога."""
    if values is None:
        return []
    ...
    selected = {str(value).strip() for value in values if str(value).strip() in PROFILE_INTEREST_KEYS}
    return [key for key, _label in PROFILE_INTERESTS if key in selected][:PROFILE_INTEREST_MAX]

def _profile_interests_decode(raw) -> list[str]:
    if not raw:
        return []
    try:
        ...
        values = []
    return normalize_profile_interests(values if isinstance(values, list) else [])

def _profile_interests_encode(values) -> str:
    return json.dumps(normalize_profile_interests(values), ensure_ascii=False)

def format_profile_interests(values, limit: int | None = None) -> str:
    keys = normalize_profile_interests(values)
    if not keys:
        return "— пока не указаны —"
    shown = keys if limit is None else keys[:max(0, int(limit))]
    labels = [PROFILE_INTEREST_LABELS[key] for key in shown]
...
# ------- HELP MENU: анкеты -------
    ...
    # Интересы сотрудника хранятся как JSON-массив стабильных ключей.
    try:
        cur.execute("ALTER TABLE profiles ADD COLUMN interests_json TEXT NOT NULL DEFAULT '[]'")
    except sqlite3.OperationalError:
        pass
    cur.execute("UPDATE profiles SET interests_json='[]' WHERE interests_json IS NULL OR interests_json=''")
...
def db_profiles_upsert(
    ...
    if row:
        ...
        cur.execute(
            ...
            (
                ...
                topics.strip(), (tg_link or "").strip(),
                _profile_interests_encode(interests) if interests is not None else None,
                photo_file_id, pid,
            ),
        )
        con.commit()
    ...
    cur.execute(
        """INSERT INTO profiles(
               ...
               photo_file_id, interests_json, created_at
           ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
            full_name.strip(), int(year_start), city.strip(), birthday, about.strip(),
            topics.strip(), (tg_link or "").strip(), photo_file_id,
            _profile_interests_encode(interests), datetime.utcnow().isoformat(),
        ),
    )
    con.commit()
    pid = cur.lastrowid
...
def db_profiles_with_shared_interests(profile_id: int) -> list[dict]:
    ...
    if not current:
        return []
    own = set(normalize_profile_interests(current.get("interests")))
    if not own:
        return []
    ...
    cur = con.cursor()
    cur.execute(
        "SELECT id, full_name, interests_json FROM profiles "
...
def db_profiles_get(pid: int):
    ...
    cur.execute("""
        SELECT id, full_name, year_start, city, birthday, about, topics, tg_link,
               tg_user_id, avg_test_score, photo_file_id, interests_json
        FROM profiles
        WHERE id=?
    """, (pid,))
    row = cur.fetchone()
    ...
    return {
        ...
        "interests": _profile_interests_decode(row[11]),
    }
...
def db_profiles_add(
    ...
    """, (
        ...
        topics.strip(), tg_link.strip(), photo_file_id,
        _profile_interests_encode(interests), datetime.utcnow().isoformat(),
    ))
    con.commit()
    pid = cur.lastrowid
...
def db_profiles_update(
    ...
    if keep_existing_photo and photo_file_id is None:
        cur.execute(
            """
            UPDATE profiles
            SET full_name=?, year_start=?, city=?, birthday=?, about=?, topics=?, tg_link=?, interests_json=?
            WHERE id=?
            """,
            (
                full_name.strip(), int(year_start), city.strip(), (birthday or None), about.strip(),
                topics.strip(), tg_link.strip(), _profile_interests_encode(interests), int(pid),
            ),
        )
    else:
        cur.execute(
            ...
            UPDATE profiles
            SET full_name=?, year_start=?, city=?, birthday=?, about=?, topics=?, tg_link=?,
                photo_file_id=?, interests_json=?
            WHERE id=?
            """,
            (
                full_name.strip(), int(year_start), city.strip(), (birthday or None), about.strip(),
                topics.strip(), tg_link.strip(), photo_file_id,
                _profile_interests_encode(interests), int(pid),
            ),
        )
    ok = cur.rowcount > 0
    con.commit()
...
def kb_my_profile_edit_fields():
    ...
    return InlineKeyboardMarkup([
        ...
        [
            ...
            InlineKeyboardButton(PROFILE_SELF_EDIT_FIELDS["interests"], callback_data="help:me:edit:interests"),
        ],
        [
...
def profile_self_edit_prompt(field: str, profile: dict) -> str:
    ...
    prompts = {
        ...
        "interests": "Выберите до 5 интересов кнопками ниже.",
    ...
    if field == "birthday":
        ...
        current = format_profile_interests(profile.get("interests"))
    elif field == "photo":
...
def kb_profile_interests(selected=None):
    selected_keys = set(normalize_profile_interests(selected))
    rows = []
    for index in range(0, len(PROFILE_INTERESTS), 2):
        row = []
        for key, label in PROFILE_INTERESTS[index:index + 2]:
            prefix = "✅ " if key in selected_keys else ""
            row.append(
                InlineKeyboardButton(
                    prefix + label,
                    callback_data=f"help:profile:interests:toggle:{key}",
                )
            )
        rows.append(row)
    rows.append([
        InlineKeyboardButton("🗑 Очистить", callback_data="help:profile:interests:clear"),
        InlineKeyboardButton("✅ Готово", callback_data="help:profile:interests:done"),
    ])
...
def kb_help_team(page: int = 0, can_create_profile: bool = False):
    ...
    rows.append([
        InlineKeyboardButton(
            "✨ Коллеги со схожими интересами",
            callback_data="help:team:similar:0",
        )
    ])
...
def export_backup_zip_bytes() -> bytes:
    ...
    cur = con.cursor()
    cur.execute("""
        SELECT id, full_name, year_start, city, birthday, about, topics, interests_json, tg_link,
    ...
    for row in cur.fetchall():
        w.writerow({
            ...
            "about": row[5] or "",
            "topics": row[6] or "",
            "interests_json": row[7] or "[]",
            "tg_link": row[8] or "",
            "tg_user_id": row[9] if row[9] is not None else "",
            "is_active": int(row[10]) if row[10] is not None else 1,
            "avg_test_score": row[11] if row[11] is not None else "",
            "photo_file_id": row[12] or "",
        })
    con.close()
    ...
    with zipfile.ZipFile(zbuf, "r") as zf:
        ...
        if "profiles.csv" in names:
            ...
            for row in rdr:
                ...
                interests = _profile_interests_decode((row.get("interests_json") or "[]").strip())
                ...
                if pid.isdigit():
                    cur.execute(
                        """INSERT INTO profiles(
                               id, full_name, year_start, city, birthday, about, topics, interests_json, tg_link,
                        ...
                        (
                            ...
                            _profile_interests_encode(interests), tg_link, tg_user_id, is_active,
                            avg_test_score, photo_file_id, created_at,
                        ),
                    )
                    new_id = int(pid)
                else:
                    ...
                    if new_id is None:
                        cur.execute(
                            """INSERT INTO profiles(
                                   full_name, year_start, city, birthday, about, topics, interests_json, tg_link,
                                   tg_user_id, is_active, avg_test_score, photo_file_id, created_at
                               ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                            """,
                            (
                                full_name, int(year_start), city, birthday, about, topics,
                                _profile_interests_encode(interests), tg_link, tg_user_id, is_active,
                                avg_test_score, photo_file_id, created_at,
                            ),
                        )
...
def export_backup_csv_bytes() -> bytes:
    ...
    cur = con.cursor()
    cur.execute("""
        SELECT full_name, year_start, city, birthday, about, topics, interests_json, tg_link
        FROM profiles
        ORDER BY id ASC
    """)
    profs = cur.fetchall()
    ...
    for p in profs:
        full_name, year_start, city, birthday, about, topics, interests_json, tg_link = p
        writer.writerow({
            "kind": "profile",
            ...
            "profile_interests": interests_json or "[]",
...
async def cmd_export_csv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur = con.cursor()
    cur.execute("""
        SELECT id, full_name, year_start, city, birthday, about, topics, interests_json, tg_link, avg_test_score
        FROM profiles
        ORDER BY full_name COLLATE NOCASE ASC
    """)
    for row in cur.fetchall():
        writer.writerow({
            ...
            "profile_interests": row[7] or "[]",
            "profile_tg_link": row[8] or "",
            "profile_avg_test_score": row[9] if row[9] is not None else "",
        })
    con.close()
...
async def cb_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------------- Интересы анкеты: мультивыбор до 5 ----------------
    if data.startswith("help:profile:interests:"):
        if not context.chat_data.get(PROFILE_WIZ_ACTIVE) or context.chat_data.get(PROFILE_WIZ_STEP) != "interests":
        ...
        selected = normalize_profile_interests(profile_data.get("interests"))
        action = data.split(":")[3] if len(data.split(":")) > 3 else ""

        if action == "toggle":
            key = data.rsplit(":", 1)[-1]
            if key not in PROFILE_INTEREST_KEYS:
                await q.answer("Неизвестный интерес.", show_alert=True)
                return
            if key in selected:
                selected.remove(key)
            else:
                if len(selected) >= PROFILE_INTEREST_MAX:
                    await q.answer(f"Можно выбрать не более {PROFILE_INTEREST_MAX} интересов.", show_alert=True)
                    return
                selected.append(key)
            profile_data["interests"] = normalize_profile_interests(selected)
            context.chat_data[PROFILE_WIZ_DATA] = profile_data
            await q.edit_message_text(
                profile_interests_prompt(profile_data["interests"]),
                parse_mode=ParseMode.HTML,
                reply_markup=kb_profile_interests(profile_data["interests"]),
            )
            return
        ...
        if action == "done":
            profile_data["interests"] = normalize_profile_interests(selected)
            context.chat_data[PROFILE_WIZ_DATA] = profile_data
            if context.chat_data.get(PROFILE_WIZ_SINGLE_FIELD) == "interests":
                _ok, message_text, markup = await finalize_profile_wizard(update, context)
                await q.edit_message_text(message_text, reply_markup=markup)
                return
            context.chat_data[PROFILE_WIZ_STEP] = "topics"
            await q.edit_message_text(
                "Шаг 7/9: по каким вопросам к вам можно обращаться?",
    ...
    if data.startswith("help:me:edit:"):
        ...
        if field == "interests":
            await q.edit_message_text(
                profile_interests_prompt(profile.get("interests")),
                parse_mode=ParseMode.HTML,
                reply_markup=kb_profile_interests(profile.get("interests")),
            )
            return
    ...
    if data.startswith("help:suggest:mode:"):
        ...
        if mode not in {"anon", "named"}:
            try:
                await q.answer("Неизвестный режим предложки.", show_alert=True)
            except (TimedOut, NetworkError):
                pass
            return
        # В ЛС у пользователя может ещё не быть сохранённой привязки к чату
        # (например, если он впервые открыл /help именно там). В таком случае
        # используем основной рабочий чат, но не затираем ранее выбранную
        # групповую привязку.
        scope_chat_id = get_scope_chat_id(update, context) or ACCESS_CHAT_ID
    ...
    if data.startswith("help:team:similar:"):
        ...
        if not own_profile:
            ...
            return
        if not normalize_profile_interests(own_profile.get("interests")):
            await replace_callback_message_with_text(
                q,
                context,
                ...
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🎯 Добавить интересы", callback_data="help:me:edit:interests")],
                    [InlineKeyboardButton("⬅️ Ко всей команде", callback_data="help:team")],
                ]),
            )
            return
    ...
    if context.chat_data.get(WAITING_CSV_IMPORT):
        ...
        for row in reader:
            ...
            if kind == "profile":
                ...
                interests = _profile_interests_decode((row.get("profile_interests") or "[]").strip())
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get(WAITING_SUGGESTION_TEXT):
        ...
        if sent_ok <= 0:
            await update.message.reply_text(
                "⚠️ Не удалось передать сообщение тимлиду. "
                "Попробуйте ещё раз чуть позже."
            )
            return

        # Ограничиваем повторную отправку только после успешной доставки хотя
        # бы одному администратору. Раньше лимит выставлялся даже при полном
        # провале отправки, из-за чего «Предложка» казалась сломанной.
        if user_id:
            db_set_suggest_last_ts(user_id, int(time.time()))

        if sent_fail:
            await update.message.reply_text(
                f"✅ Передал тимлиду ({sent_ok}). "
                "Часть администраторов сейчас недоступна."
            )
        else:
    ...
    if context.chat_data.get(PROFILE_WIZ_ACTIVE):
        ...
        if step == "full_name":
            ...
            await update.message.reply_text("Шаг 2/9: с какого года работает? Пример: 2022", reply_markup=kb_cancel_wizard_settings())
            return
        ...
        if step == "year_start":
            ...
            await update.message.reply_text("Шаг 3/9: город проживания. Пример: Москва", reply_markup=kb_cancel_wizard_settings())
            return
        ...
        if step == "city":
            ...
            await update.message.reply_text(
                "Шаг 4/9: день рождения (формат <b>ДД.ММ</b>)\n"
        ...
        if step == "birthday":
            ...
            await update.message.reply_text("Шаг 5/9: кратко о себе (1–3 предложения)", reply_markup=kb_cancel_wizard_settings())
            return
        ...
        if step == "about":
            ...
            if await finalize_single_profile_field_if_needed(update, context, "about"):
                return
            context.chat_data[PROFILE_WIZ_STEP] = "interests"
            data["interests"] = normalize_profile_interests(data.get("interests"))
            context.chat_data[PROFILE_WIZ_DATA] = data
            await update.message.reply_text(
                "Шаг 6/9\n\n" + profile_interests_prompt(data.get("interests")),
                parse_mode=ParseMode.HTML,
                reply_markup=kb_profile_interests(data.get("interests")),
            )
            return

        if step == "interests":
            await update.message.reply_text(
                "Выберите интересы кнопками под предыдущим сообщением.",
                reply_markup=kb_profile_interests(data.get("interests")),
            )
            return
        ...
        if step == "topics":
            ...
            await update.message.reply_text("Шаг 8/9: Telegram (@username или https://t.me/username)", reply_markup=kb_cancel_wizard_settings())
            return
        ...
        if step == "tg_link":
            ...
            text_photo = (
                "Шаг 9/9: отправьте <b>фотографию сотрудника</b>.\n\n"
...
async def send_suggestion_to_admins(scope_chat_id: int, update: Update, context: ContextTypes.DEFAULT_TYPE, message_text: str, mode: str) -> tuple[int, int]:
    ...
    # Сообщение администратору отправляется в HTML-режиме. Экранируем все
    # динамические поля, иначе символы вроде "&" или "<" ломают запрос
    # Telegram и вся функция «Предложка» выглядит неработающей.
    chat_title_html = html_lib.escape(str(chat_title), quote=False)
    raw_text = (message_text or "").strip()
    if not raw_text:
        return (0, 0)

    # Telegram ограничивает длину сообщения 4096 символами. Оставляем запас
    # под заголовок и безопасно обрезаем исходный текст до формирования HTML.
    ...
    admin_text = (
        ...
        f"Чат: <b>{chat_title_html}</b> (<code>{int(scope_chat_id)}</code>)\n"
        f"{sender_line}\n\n"
        f"Сообщение:\n{message_html}"
    )
...
def clear_industry_specialist_flow(context: ContextTypes.DEFAULT_TYPE) -> None:
    context.user_data.pop(INDUSTRY_SPECIALIST_STATE, None)
    context.user_data.pop(INDUSTRY_SPECIALIST_DRAFT, None)


def industry_specialist_industry_items() -> list[tuple[str, str]]:
    """Единый список отраслей для раздела и административного редактора."""
    return [(str(key), str(title)) for key, title, _url in INDUSTRY_DIVISION_ITEMS]


def industry_specialist_industry_title(industry_key: str) -> str:
    item = INDUSTRY_DIVISION_BY_KEY.get(str(industry_key))
...
def industry_specialist_tg_url(value: str | None) -> str | None:
    mention = industry_specialist_normalize_tg_link(value)
    return f"https://t.me/{mention[1:]}" if mention else None
...
def db_init():
    ...
    with sqlite3.connect(DB_PATH) as con:
        con.execute(
            """
            CREATE TABLE IF NOT EXISTS industry_specialists (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                full_name TEXT NOT NULL,
                industry_key TEXT NOT NULL,
...
def db_industry_specialist_add(
    ...
    with sqlite3.connect(DB_PATH) as con:
        cur = con.execute(
            """
            INSERT INTO industry_specialists(
...
def kb_industry_specialist_card(item: dict, source: str = "all") -> InlineKeyboardMarkup:
    rows = []
    tg_url = industry_specialist_tg_url(item.get("telegram_link"))
    if tg_url:
        rows.append([InlineKeyboardButton("✈️ Написать в Telegram", url=tg_url)])
    if source == "tools":
        rows.append([
            InlineKeyboardButton(
                "⬅️ К инструментам",
...
def kb_industry_specialists_admin() -> InlineKeyboardMarkup:
    ...
    for item in db_industry_specialists_list():
        rows.append([
            InlineKeyboardButton(
                f"✏️ {item['full_name'][:52]}",
                callback_data=f"help:ispec:open:{int(item['id'])}",
            )
        ])
    rows.append([InlineKeyboardButton("⬅️ Назад", callback_data="help:settings:content")])
    return InlineKeyboardMarkup(rows)
...
def kb_industry_specialist_admin_card(item: dict) -> InlineKeyboardMarkup:
...
def kb_industry_division_tools(user_id: int | None = None) -> InlineKeyboardMarkup:
    ...
    insert_at = next(
        (
            index for index, row in enumerate(rows)
            if any((button.callback_data or "") == "help:industry_division" for button in row)
        ),
        len(rows),
    )
    specialist_rows = [
        [
            InlineKeyboardButton(
                f"👤 {item['full_name'][:55]}",
                callback_data=f"help:industry_specialist:{int(item['id'])}:tools",
            )
        ]
        for item in specialists
    ]
    rows[insert_at:insert_at] = specialist_rows
    return InlineKeyboardMarkup(rows)
...
async def cb_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("help:industry_specialist:"):
        if await deny_no_access(update, context):
            return
        parts = data.split(":")
        try:
            ...
            return
        source = parts[3] if len(parts) > 3 else "all"
        ...
        try:
            ...
            pass
        await query.edit_message_text(
            industry_specialist_card_text(item),
            parse_mode=ParseMode.HTML,
            reply_markup=kb_industry_specialist_card(item, source),
            disable_web_page_preview=True,
        )
        return

    if data.startswith("help:ispec"):
        if not await is_admin_scoped(update, context):
            try:
        ...
        if data == "help:ispec":
            ...
            await query.edit_message_text(
                industry_specialists_admin_text(),
                parse_mode=ParseMode.HTML,
                reply_markup=kb_industry_specialists_admin(),
            )
            return

        if data == "help:ispec:add":
            clear_industry_specialist_flow(context)
        ...
        if data == "help:ispec:savei:add":
            ...
            await query.edit_message_text(
                ...
                "Отправьте <code>@username</code>, имя пользователя или ссылку "
                "<code>https://t.me/username</code>.",
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("❌ Отмена", callback_data="help:ispec")
        ...
        if data.startswith("help:ispec:open:"):
            specialist_id = int(data.rsplit(":", 1)[-1])
            item = db_industry_specialist_get(specialist_id)
            if not item:
                await query.edit_message_text(
        ...
        if data.startswith("help:ispec:editn:"):
            specialist_id = int(data.rsplit(":", 1)[-1])
            item = db_industry_specialist_get(specialist_id)
            if not item:
                await query.edit_message_text(
        ...
        if data.startswith("help:ispec:editi:"):
            specialist_id = int(data.rsplit(":", 1)[-1])
            item = db_industry_specialist_get(specialist_id)
            if not item:
                await query.edit_message_text(
        ...
        if data.startswith("help:ispec:editt:"):
            specialist_id = int(data.rsplit(":", 1)[-1])
            item = db_industry_specialist_get(specialist_id)
            if not item:
                await query.edit_message_text(
        ...
        if data.startswith("help:ispec:delask:"):
            specialist_id = int(data.rsplit(":", 1)[-1])
            item = db_industry_specialist_get(specialist_id)
            if not item:
                await query.edit_message_text(
        ...
        if data.startswith("help:ispec:del:"):
            specialist_id = int(data.rsplit(":", 1)[-1])
            deleted = db_industry_specialist_delete(specialist_id)
            clear_industry_specialist_flow(context)
            await query.edit_message_text(
                "✅ Специалист удалён." if deleted else "Специалист уже удалён.",
                reply_markup=kb_industry_specialists_admin(),
            )
            return

    return await _industry_specialists_legacy_cb_help(update, context)
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = re.sub(r"\s+", " ", (update.message.text or "").strip())
    draft = dict(context.user_data.get(INDUSTRY_SPECIALIST_DRAFT) or {})
