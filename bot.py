...
def kb_horo_actions(sign_slug: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton(MEME_BUTTON_TEXT, callback_data=f"horo:meme:{sign_slug}")],
            [InlineKeyboardButton("Тест: сгенерировать 3 мема 🔁", callback_data=f"horo:meme_test:{sign_slug}")],
        ]
    )
...
async def _send_horo_dm(user_id: int, sign_slug: str, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        parse_mode=ParseMode.HTML,
        disable_web_page_preview=True,
        reply_markup=kb_horo_actions(sign_slug),
    )
...
async def cb_horo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action not in ("sign", "meme", "meme_test"):
        return
    ...
    if action == "meme_test":
        try:
            # отправляем 3 мема подряд — удобно проверить нагрузку/стабильность
            for _ in range(3):
                await send_personal_meme_dm(user_id, sign_slug, user.first_name, context)
